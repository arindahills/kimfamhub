    ...
    model {
        ...
        kimfam = softwareSystem "KimFam Hub" "Family investment club portal" {
            ...
            webapp = container "FastAPI Web App" "Gunicorn/Uvicorn. Auth, JSON API, SSE, serves the built SPA." "Python / FastAPI" {
                ...
                sheepTracker     = component "Sheep Tracker" "sheep.py — native Postgres tracker (sheep_animals/events/expenses): flock, mortality, categorized expenses, capital/sales; idempotent+atomic seed; live /api/projects/sheep/detail. NOT Google Sheets. See ADR-026, Epic #16"
                klafamApi   = component "KlaFam Tanda API"  "/api/klafam/* — rotating savings (ROSCA): cycles + rotation, own contribution (pay/offset), beneficiary-only cycle acknowledge, and beneficiary/admin record-for-member receipt carrying recorded_by attribution. See ADR-027"
